...
- **The clerk**, the section "Put the judge on the stand", between the feed and
  ...
  committed case. It says plainly that this is not the chain.* It replaces the
  withdraw shot at 1:12 if both are wanted and there is no room.
...
4. **Browser tab 2, the feed** (`#feed`), loaded, showing the rows from a
   rehearsal so the table is not empty at 1:04. It will refresh once the
   contested payment lands.
...
8. The linter service, if the OPTIONAL linter or clerk shot is being recorded:
...
record the third. The recording writes new payments to the chain, so take the
final snapshot after it, not before: `python scripts/snapshot.py`, then
