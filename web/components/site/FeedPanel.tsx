...
export default function FeedPanel({ data, limit = 6 }: { data: FeedData; limit?: number }) {
    ...
    return {
      ...
      toggle: () => {
        ...
        if (!isOpen && row.case && !strings[row.pid]) {
          ...
          fetch(`/api/case/${row.pid}${networkQuery(data.network)}`)
  ...
  const v = {
    ...
    stat2Color: known ? "#EEF3F8" : "#7C8798",
    stat3Color: known ? "#F87171" : "#7C8798",
